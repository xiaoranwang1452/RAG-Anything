...
# --- FIXED: make embedding a single-text async function ---
async def embed_one(text: str) -> List[float]:
    # Ollama client is synchronous; run in thread to avoid blocking event loop
    def _call():
...
async def demonstrate_reflection_layer():
    ...
    ENABLE_TABLE = False  # disabled to simplify first run
    ENABLE_EQUATION = False  # disabled to simplify first run
    ENABLE_IMAGE = _getenv_bool("ENABLE_IMAGE_PROCESSING", False)
    ...
    try:
        ...
        print("🏗️ Initializing RAGAnything...")
        rag = RAGAnything(
            config=config,
        ...
        core_keys = [
            ...
            ("coverage", "📈 Coverage"),
            ("contradiction_ratio", "⚠️ Contradiction Ratio"),
